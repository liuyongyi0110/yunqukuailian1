...
import com.yunqukuailian.app.view.simplekview.EntityImpl.KLineImpl;
import com.yunqukuailian.app.view.simplekview.base.IAdapter;
...
import com.yunqukuailian.app.view.simplekview.formatter.TimeFormatter;
...
public abstract class BaseKChartView extends ScrollAndScaleView {
    ...
    private void initRect(int w,int h)
    {
        ...
        int mMainHeight = (int) (displayHeight );
        ...
        mChildRect=new Rect(0,mTabRect.bottom,mWidth,mTabRect.bottom);
    }
...
//        drawValue(canvas, isLongPress ? mSelectedIndex : mStopIndex);
    ...
    private void drawGird(Canvas canvas) {
        ...
        for (int i = 0; i <= mGridRows; i++) {
            ...
            canvas.drawLine(0, rowSpace * i+mMainRect.top, (getWidth()-getWidth()*2/(3*mGridColumns)), rowSpace * i+mMainRect.top, mGridPaint);
        }
        //-----------------------下方子图------------------------
...
//        canvas.drawLine(0, mChildRect.bottom, mWidth, mChildRect.bottom, mGridPaint);
    ...
    private void drawK(Canvas canvas) {
        ...
        float minus = (getWidth()-getWidth()*2/(3*mGridColumns))/(7*mScaleX*mPointWidth);
        for (int i = mStartIndex; i < mStopIndex -minus; i++) {
...
//            canvas.drawText(formatValue(mMainMinValue), 0, mMainRect.bottom-textHeight+baseLine, mTextPaint);
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, (getWidth()-getWidth()*2/(3*mGridColumns)), fixTextY(rowSpace * i+mMainRect.top), mTextPaint);
            }
        }
        //--------------画下方子图的值-------------
//        if (mChildDraw != null) {
...
//        }
        ...
        for (int i = 1; i < mGridColumns; i++) {
            ...
            if (translateX >= startX && translateX <= stopX) {
                ...
                canvas.drawText(text, columnSpace * i - mTextPaint.measureText(text) / 2, y+mTextPaint.getTextSize(), mTextPaint);
            }
        }

        float translateX = xToTranslateX(0);
        if (translateX >= startX && translateX <= stopX) {
            canvas.drawText(formatDateTime(getAdapter().getDate(mStartIndex)), 0, y+mTextPaint.getTextSize(), mTextPaint);
        }
        translateX = xToTranslateX(mWidth);
        if (translateX >= startX && translateX <= stopX) {
            String text = formatDateTime(getAdapter().getDate(mStopIndex));
            canvas.drawText(text, mWidth - mTextPaint.measureText(text), y+mTextPaint.getTextSize(), mTextPaint);
        }
        if (isLongPress) {
            KLineImpl point = (KLineImpl) getItem(mSelectedIndex);
